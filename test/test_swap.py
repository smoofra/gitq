...
def test_keep_going(repo: Git):
    ...
    repo.s("git commit -q -m B")
    sha = repo.sha("HEAD")
    repo.s("git swap --keep-going --no-edit")
    assert repo.t(f"git diff --exit-code {sha} HEAD")
...
def test_keep_going_upstream_force(repo: Git):
    for c in "abcde":
        repo.w(c, c)
        repo.s("git add .")
        repo.s(f"git commit -q -m {c}")
    ...
    sha = repo.sha("HEAD")
    assert repo.log() == ["a", "b", "c", "d", "e", "X"]
    repo.s("git swap --keep-going --force")
    assert repo.t(f"git diff --exit-code {sha} HEAD")
    assert repo.log() == ["X", "a", "b", "c", "d", "e"]
...
def test_middle_keep_going(repo: Git):
    ...
    sha = repo.sha("HEAD")
    repo.s("git swap --keep-going --no-edit :/B")
    assert repo.t(f"git diff --exit-code {sha} HEAD")
...
def test_keep_going_up(repo: Git):
    ...
    repo.s("git swap --keep-going --up --no-edit :/a")
...
def test_keep_going_up_stop(repo: Git):
    ...
    repo.s("git swap --keep-going --up :/a; [[ $? = 2 ]]")
    repo.s("git swap --stop")
